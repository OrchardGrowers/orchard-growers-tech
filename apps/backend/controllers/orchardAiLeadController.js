...
const normalizeCollectorText = (value, fallback = "") =>
  typeof value === "string" && value.trim() ? value.trim() : fallback;

const buildCollectorSeedLeads = ({ fruit, state, city, leadType, query }) => {
  const cleanFruit = normalizeCollectorText(fruit, "Apple");
  const cleanState = normalizeCollectorText(state, "Himachal Pradesh");
  const cleanCity = normalizeCollectorText(city, "");
  const cleanLeadType = normalizeCollectorText(leadType, "Buyer");
  const cleanQuery = normalizeCollectorText(query, `${cleanFruit} ${cleanLeadType} ${cleanState}`);

  const locationLabel = [cleanCity, cleanState].filter(Boolean).join(", ");
  const baseName = `${cleanFruit} ${cleanLeadType}`;

  return [
    {
      companyName: `${baseName} Lead - ${cleanState}`,
      ...
      whatsapp: "",
      website: "",
      sourceUrl: `local-seed://${encodeURIComponent(cleanQuery)}`,
      sourcePlatform: "Orchard AI Local Collector",
      score: 35,
      priority: "Medium",
      status: "New",
      assignedTo: null,
      notes: `Seed lead generated by Orchard AI Collector. Search query: ${cleanQuery}. Verify contact details before outreach.`,
      tags: ["ai-collected", "needs-verification", cleanFruit.toLowerCase(), cleanLeadType.toLowerCase()],
    },
  ];
};

export const collectOrchardAiLeads = async (req, res) => {
  const seedLeads = buildCollectorSeedLeads(req.body || {});
  const created = [];
  const skipped = [];

  for (const seed of seedLeads) {
    const payload = sanitizeLeadPayload(seed);
    try {
      await checkForDuplicateLead(payload);
      const lead = await Lead.create({
        ...payload,
      ...
      if (error.code === "DUPLICATE_LEAD" || error.code === 11000) {
        skipped.push({
  ...
  return res.status(201).json({
    message: `Collector finished. Created ${created.length}, skipped ${skipped.length}.`,
    data: created,
    skipped,
  });
};
